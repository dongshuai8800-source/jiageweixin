...
export default function App() {
  ...
  const reloadUnifiedData = async (notify = false) => {
    ...
    try {
      ...
      if (notify) triggerAlertToast(error instanceof Error ? error.message : "统一数据源加载失败，已回退到内置数据");
    } finally {
  ...
  const activeData = useMemo(() => {
    ...
    return {
      ...
      activeRate: city.doctors !== null && city.doctors > 0 && city.activeDoctors !== null ? parseFloat(((city.activeDoctors / city.doctors) * 100).toFixed(1)) : null,
      ...
      activeGroupChatsRate: city.groupChats !== null && city.groupChats > 0 && city.activeGroupChats !== null ? parseFloat(((city.activeGroupChats / city.groupChats) * 100).toFixed(1)) : null,
      groupMembers: city.groupMembers,
      activeGroupMembers: city.activeGroupMembers,
      activeGroupMembersRate: city.groupMembers !== null && city.groupMembers > 0 && city.activeGroupMembers !== null ? parseFloat(((city.activeGroupMembers / city.groupMembers) * 100).toFixed(1)) : null,
      ...
      <div className="max-w-[1600px] mx-auto space-y-3 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 items-stretch" id="dashboard-columns-grid">
          <section className="lg:col-span-4 flex flex-col gap-3">
            <div className="bg-[#0b1329]/80 border border-slate-800 p-3 rounded-xl flex flex-col gap-2.5 shadow-[0_4px_25px_rgba(0,0,0,0.4),inset_0_0_15px_rgba(6,182,212,0.05)]"><div className="flex items-center justify-between border-b pb-1 border-slate-800/80"><h3 className="font-bold text-xs tracking-wider flex items-center gap-1.5 text-cyan-400 uppercase"><Building2 className="w-3.5 h-3.5" />机构概况</h3><span className="text-[9px] font-mono text-gray-500">PROFILES</span></div><div className="grid grid-cols-3 gap-1.5 pb-0.5">{profileCards.map((card) => <div key={card.label} className="p-2 rounded bg-slate-950/40 border border-slate-800/80 text-center"><span className="text-[9px] text-gray-500 block leading-none">{card.label}</span><div className="mt-1 flex items-baseline justify-center gap-0.5"><span className={`text-base font-extrabold font-mono ${card.color}`}>{fmt(card.value)}</span><span className="text-[8px] text-gray-500">{card.unit}</span></div></div>)}</div><div className="bg-cyan-950/15 border border-cyan-500/10 p-2 rounded flex items-center justify-between"><div className="flex flex-col"><span className="text-[9px] text-gray-400">医护人员激活率</span><span className="text-[8px] text-gray-500">已激活及签约占比</span></div><div className="flex items-center gap-2"><span className="text-[13px] font-black font-mono text-teal-400">{fmt(activeData.activeRate, "%")}</span><div className="w-14 bg-slate-900 border border-slate-800 h-1.5 rounded-full overflow-hidden"><div className="bg-teal-500 h-full rounded-full" style={{ width: `${activeData.activeRate ?? 0}%` }} /></div></div></div></div>
